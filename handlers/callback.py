...
async def send_album(callback: CallbackQuery):
    ...
    await callback.message.answer_photo(photo = 'https://www.multitour.ru/files/out/topplace-moscow/Moskovskii_universitet/Moskovskii_universitet_3.jpg', caption='1️⃣ <b>️МГУ</b>\nГлавное здание на Воробьёвых горах. Это одна из семи сталинских высоток, символ российской науки.', parse_mode='HTML')
    await callback.message.answer_photo(photo = 'https://ekskursii-v-moskve.ru/wp-content/uploads/2020/12/ekskusrii-po-moskve.jpg', caption='2️⃣ <b>️Красная площадь </b>\nЭто исторический, культурный и туристический центр Москвы и всей России. Архитектурный ансамбль Красной площади и Московского Кремля охраняется ЮНЕСКО как памятник Всемирного наследия.', parse_mode='HTML')
    await callback.message.answer_photo(photo = 'https://www.classicalmusicnews.ru/wp-content/uploads/2025/01/bolshoj-teatr4.jpg', caption='3️⃣ <b>️Большой театр</b>\nОдин из крупнейших и старейших в России и один из самых значительных в мире театров оперы и балета. Комплекс зданий театра расположен в центре Москвы, на Театральной площади.', parse_mode='HTML')
    await callback.message.answer(text='Минимальная цена на недельный тур от 30 000₽')
    await callback.message.answer(text='Смог ли я помочь тебе?', reply_markup=yes_no_kb)
...
async def send_album(callback: CallbackQuery):
    ...
    await callback.message.answer_photo(photo = 'https://7d9e88a8-f178-4098-bea5-48d960920605.selcdn.net/4aceed97-dd0f-44a0-a2c1-fc9f93a1e13a/-/format/webp/-/resize/1300x/', caption='3️⃣ <b>Дворцовая площадь </b> \nЭто место считается первым исторически сложившимся архитектурным ансамблем. На дворцовой площади находится Зимний дворец, который ранее был императорским. ', parse_mode='HTML')
    await callback.message.answer(text='Минимальная цена на недельный тур от 24 000₽ из Москвы')
    await callback.message.answer(text='Смог ли я помочь тебе?', reply_markup=yes_no_kb)
...
async def send_album(callback: CallbackQuery):
    ...
    await callback.message.answer(text='Минимальная цена на недельный тур от 45 000₽ из Москвы')
    await callback.message.answer(text='Смог ли я помочь тебе?', reply_markup=yes_no_kb)
...
async def send_album(callback: CallbackQuery):
    ...
    await callback.message.answer(text='Минимальная цена на недельный тур от 260 000₽ из Москвы')
    await callback.message.answer(text='Смог ли я помочь тебе?', reply_markup=yes_no_kb)
...
async def send_album(callback: CallbackQuery):
    ...
    await callback.message.answer(text='Минимальная цена на недельный тур от 250 000₽ из Москвы')
    await callback.message.answer(text='Смог ли я помочь тебе?', reply_markup=yes_no_kb)
...
async def send_album(callback: CallbackQuery):
    ...
    await callback.message.answer(text='Минимальная цена на недельный тур от 270 000₽ из Москвы')
    await callback.message.answer(text='Смог ли я помочь тебе?', reply_markup=yes_no_kb)
...
async def send_album(callback: CallbackQuery):
    ...
    await callback.message.answer(text='Минимальная цена на недельный тур от 115 000₽ из Москвы')
    await callback.message.answer(text='Смог ли я помочь тебе?', reply_markup=yes_no_kb)
...
async def send_album(callback: CallbackQuery):
    ...
    await callback.message.answer(text='Минимальная цена на недельный тур от 130 000₽ из Москвы')
    await callback.message.answer(text='Смог ли я помочь тебе?', reply_markup=yes_no_kb)
...
async def send_album(callback: CallbackQuery):
    ...
    await callback.message.answer(text='Минимальная цена на недельный тур от 120 000₽ из Москвы')
    await callback.message.answer(text='Смог ли я помочь тебе?', reply_markup=yes_no_kb)
...
async def send_album(callback: CallbackQuery):
    ...
    await callback.message.answer(text='Минимальная цена на недельный тур от 105 000₽ из Москвы')
    await callback.message.answer(text='Смог ли я помочь тебе?', reply_markup=yes_no_kb)
...
async def send_album(callback: CallbackQuery):
    ...
    await callback.message.answer_photo(photo = 'https://dynamic-media-cdn.tripadvisor.com/media/photo-o/02/69/65/ef/praia-de-copacabana-ao.jpg?w=900&h=500&s=1', caption='2️⃣ <b>Morro da Urca</b> \n  Гора - прекрасный вид и хорошее соотношение цены и качества для поездки на канатной дороге до вершины, а затем до Сахарной головы. ', parse_mode='HTML')
    await callback.message.answer(text='Минимальная цена на недельный тур от 170 000₽ на человека из Москвы')
    await callback.message.answer(text='Смог ли я помочь тебе?', reply_markup=yes_no_kb)

@callback_router.callback_query(F.data == 'yes')
async def say_bye(callback: CallbackQuery):
    await callback.message.edit_text(text='Я рад! До встречи 🤍')

@callback_router.callback_query(F.data == 'no')
async def do_return(callback: CallbackQuery):
    await callback.message.edit_text(' Выбери страну для путешествия.', reply_markup=country_kb1)
    await callback.answer()